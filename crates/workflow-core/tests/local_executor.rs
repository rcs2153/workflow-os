...
"
    ...
    fn write_authoritative_project_validation_project(&self) {
    ...
    fn write_core_owned_authoritative_project_validation_project(&self) {
        self.write_authoritative_project_validation_project();
        self.declare_authoritative_project_validation();
        let workflow_path = self.path().join("workflows/main.workflow.yml");
        let workflow = fs::read_to_string(&workflow_path)
            .expect("authoritative workflow fixture reads")
            .replace(
  ...
  - id: echo-2
    ...
    terminal_behavior: fail_workflow",
        ...
        fs::write(workflow_path, workflow).expect("one-step authoritative fixture writes");
    }

    fn write_core_owned_authoritative_approval_project(&self) {
        self.write_core_owned_authoritative_project_validation_project();
        let workflow_path = self.path().join("workflows/main.workflow.yml");
        let workflow = fs::read_to_string(&workflow_path)
            .expect("authoritative workflow fixture reads")
            .replace(
    ...
    fn write_core_owned_authoritative_visible_project(&self) {
        ...
        fs::write(workflow_path, workflow).expect("visible fixture writes");
    }
...
"
    ...
    fn core_owned_authoritative_request(
        &self,
        run_id: WorkflowRunId,
    ) -> LocalExecutionWithCoreOwnedAuthoritativeDocsCheckGovernanceRequest {
        LocalExecutionWithCoreOwnedAuthoritativeDocsCheckGovernanceRequest {
            execution: self.immutable_bundle_request(
                run_id,
                "bundle/core-owned-authoritative-project-validation",
            ),
            selected_step_id: StepId::new("echo-1").expect("step id"),
            profile: GovernanceStrictnessProfile::ObserveAndReport,
            expected_aggregate_fingerprint: None,
            project_authoritative_execution: AuthoritativeExecutionConfiguration::new(
                GovernanceStrictnessProfile::ObserveAndReport,
                ExplicitLocalCheckProfileId::WorkflowOsProjectValidation,
            )
            .expect("supported authoritative configuration"),
...
fn core_owned_authoritative_governance_report_request(
    execution: LocalExecutionWithCoreOwnedAuthoritativeDocsCheckGovernanceRequest,
    result_id: &str,
) -> LocalExecutionWithCoreOwnedAuthoritativeGovernanceReportRequest {
    let mut report = report_inputs();
    report.local_check_result_references.clear();
...
fn live_sandbox_approval_authority_request<'a>(
    target_proof: &'a ProviderWriteSandboxTargetProof,
    attempted_record: &'a SideEffectRecord,
    run: &'a workflow_core::WorkflowRun,
) -> GitHubPrCommentLiveSandboxApprovalAuthorityCompositionRequest<'a> {
    let mut live_sandbox = live_sandbox_runtime_request(target_proof, attempted_record);
    live_sandbox.live_sandbox.readiness.approval_posture =
...
#[test]
fn current_runtime_fact_approval_resume_grant_uses_fresh_source_facts() {
    let project = TestProject::new("current-runtime-fact-approval-grant");
    project.write_approval_project();
    let calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::clone(&calls),
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-current-facts-approval-grant").expect("run id");
    let execution = project.current_runtime_fact_governance_request(
        run_id.clone(),
        "bundle/current-facts-approval-grant",
    );
    let source = CurrentRuntimeFactSource::new(vec![quiet_echo_runtime_fact()]);
    ...
    let initial_snapshot_commitment = durable_binding
        .runtime_fact_snapshot_binding()
        .expect("snapshot binding")
        .initial_snapshot_commitment()
        .clone();
    let approval = paused.run().snapshot.approval_requests[0].clone();
    let request = LocalCurrentRuntimeFactsGovernanceApprovalDecisionRequest {
        approval: project.approval_request(
            run_id,
            approval.approval_id,
            ApprovalDecisionKind::Granted,
    ...
    let completed = decide_approval_with_current_runtime_facts_governance_reassessment(
        &executor, &store, &source, request,
    )
    .expect("fresh matching facts resume execution");
...
#[test]
fn current_runtime_fact_approval_resume_changed_facts_fail_before_grant_events() {
    let project = TestProject::new("current-runtime-fact-approval-changed");
    project.write_approval_project();
    let calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::clone(&calls),
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-current-facts-approval-changed").expect("run id");
    let execution = project.current_runtime_fact_governance_request(
        run_id.clone(),
        "bundle/current-facts-approval-changed",
    );
    let source = CurrentRuntimeFactSource::new(vec![quiet_echo_runtime_fact()]);
    let paused = execute_with_current_runtime_facts_governance_assessment_binding(
        &executor, &store, &source, &execution,
    ...
    let error = decide_approval_with_current_runtime_facts_governance_reassessment(
        &executor,
        &store,
        &source,
        LocalCurrentRuntimeFactsGovernanceApprovalDecisionRequest {
    ...
    .expect_err("changed source facts fail closed");
    ...
    assert_eq!(source.calls.get(), 2);
    assert_eq!(calls.get(), 0);
    let unchanged = backend.rehydrate_run(&run_id).expect("run rehydrates");
    assert_eq!(unchanged.events, events_before);
    assert_eq!(
...
#[test]
fn current_runtime_fact_approval_resume_rejects_changed_registration_before_source_read() {
    let project = TestProject::new("current-runtime-fact-approval-registration");
    project.write_approval_project();
    let calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::clone(&calls),
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-current-facts-approval-registration").expect("run id");
    let execution = project.current_runtime_fact_governance_request(
        run_id.clone(),
        "bundle/current-facts-approval-registration",
    );
    let source = CurrentRuntimeFactSource::new(vec![quiet_echo_runtime_fact()]);
    let paused = execute_with_current_runtime_facts_governance_assessment_binding(
        &executor, &store, &source, &execution,
    ...
    let events_before = paused.run().events.clone();
    let approval = paused.run().snapshot.approval_requests[0].clone();
    let changed_registration = GovernanceRuntimeFactSourceRegistration::new(
        GovernanceRuntimeFactSourceRegistrationDefinition {
            source_id: GovernanceRuntimeFactSourceId::new("source/local-executor")
                .expect("source id"),
            contract_version: GovernanceRuntimeFactSourceContractVersion::new("v1")
    ...
    .expect_err("changed registration fails before source read");

    assert_eq!(
        error.code(),
        "executor.current_runtime_facts.approval_resume.registration_mismatch"
    );
    assert_eq!(source.calls.get(), 1);
    assert_eq!(calls.get(), 0);
    assert_eq!(
...
#[test]
fn current_runtime_fact_approval_denial_does_not_invoke_source() {
    let project = TestProject::new("current-runtime-fact-approval-denial");
    project.write_approval_project();
    let calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::clone(&calls),
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-current-facts-approval-denial").expect("run id");
    let execution = project.current_runtime_fact_governance_request(
        run_id.clone(),
        "bundle/current-facts-approval-denial",
    );
    let source = CurrentRuntimeFactSource::new(vec![quiet_echo_runtime_fact()]);
    let paused = execute_with_current_runtime_facts_governance_assessment_binding(
        &executor, &store, &source, &execution,
    ...
    let denied = decide_approval_with_current_runtime_facts_governance_reassessment(
        &executor,
        &store,
        &source,
        LocalCurrentRuntimeFactsGovernanceApprovalDecisionRequest {
    ...
    .expect("denial follows existing fail-closed path");
...
#[test]
fn current_runtime_fact_approval_resume_rejects_changed_context_before_source_read() {
    let project = TestProject::new("current-runtime-fact-approval-context");
    project.write_approval_project();
    let calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::clone(&calls),
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-current-facts-approval-context").expect("run id");
    let execution = project.current_runtime_fact_governance_request(
        run_id.clone(),
        "bundle/current-facts-approval-context",
    );
    let source = CurrentRuntimeFactSource::new(vec![quiet_echo_runtime_fact()]);
    let paused = execute_with_current_runtime_facts_governance_assessment_binding(
        &executor, &store, &source, &execution,
    ...
    let events_before = paused.run().events.clone();
    let approval = paused.run().snapshot.approval_requests[0].clone();
    let workflow_path = project.path().join("workflows/main.workflow.yml");
    let workflow = fs::read_to_string(&workflow_path).expect("workflow source");
    project.write(
        "workflows/main.workflow.yml",
        &workflow.replace("display_name: Local Main", "display_name: Changed Main"),
    );
    ...
    .expect_err("changed resolved context fails before source read");

    assert_eq!(error.code(), "executor.approval.workflow_identity_mismatch");
    assert_eq!(source.calls.get(), 1);
    assert_eq!(calls.get(), 0);
    assert_eq!(
        backend
...
#[test]
fn current_runtime_fact_approval_resume_source_failure_is_non_leaking() {
    let project = TestProject::new("current-runtime-fact-approval-source-failure");
    project.write_approval_project();
    let calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::clone(&calls),
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-current-facts-approval-source-failure").expect("run id");
    let execution = project.current_runtime_fact_governance_request(
        run_id.clone(),
        "bundle/current-facts-approval-source-failure",
    );
    ...
    assert!(!rendered.contains("bearer-secret-marker"));
    assert!(!rendered.contains("sk-live-secret-marker"));
    assert_eq!(failing_source.calls.get(), 1);
    assert_eq!(calls.get(), 0);
    assert_eq!(
        backend
            .rehydrate_run(&run_id)
...
#[test]
fn current_runtime_fact_approval_resume_requires_v3_source_commitment() {
    let project = TestProject::new("current-runtime-fact-approval-v3-required");
    project.write_approval_project();
    let calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::clone(&calls),
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-current-facts-approval-v3-required").expect("run id");
    let execution = project
        .governance_assessment_request(run_id.clone(), "bundle/current-facts-approval-v3-required");
    let paused = execute_with_governance_assessment_binding(&executor, &store, &execution)
        .expect("caller-fact execution pauses");
    let events_before = paused.run().events.clone();
    let approval = paused.run().snapshot.approval_requests[0].clone();
    let source = CurrentRuntimeFactSource::new(vec![quiet_echo_runtime_fact()]);
    ...
    .expect_err("legacy caller-fact binding is not source authority");

    assert_eq!(
        error.code(),
        "executor.current_runtime_facts.approval_resume.snapshot_commitment_missing"
    );
...
#[test]
fn explicit_project_validation_profile_uses_closed_authoritative_quiet_route() {
    let project = TestProject::new("authoritative-project-validation-profile");
...
#[test]
fn core_owned_authoritative_route_derives_facts_and_ignores_unused_visible_capability() {
    let project = TestProject::new("core-owned-authoritative-quiet");
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-core-owned-authoritative-quiet").expect("run id");
    let request = project.core_owned_authoritative_request(run_id.clone());
    let dependencies = LocalExecutionAuthoritativeVisibleGovernanceDependencies {
        disclosure: LocalExecutionGovernanceDisclosureInputs {
            delivery_id: GovernanceDisclosureDeliveryId::new(
                "delivery/core-owned-authoritative-quiet",
            )
            .expect("delivery id"),
            surface: GovernanceDisclosureSurface::new(
                GovernanceDisclosureSurfaceKind::InjectedLocal,
    ...
    let result = route_core_owned_authoritative_explicit_local_check_profile_governance(
        &executor,
        &store,
        &profile,
        Some(dependencies),
    ...
    .expect("Core-owned quiet route succeeds");
...
fn core_owned_authoritative_route_derives_visible_and_delivers_exactly_once() {
    ...
    let dependencies = LocalExecutionAuthoritativeVisibleGovernanceDependencies {
        disclosure: LocalExecutionGovernanceDisclosureInputs {
            delivery_id: GovernanceDisclosureDeliveryId::new(
                "delivery/core-owned-authoritative-visible",
            )
            .expect("delivery id"),
            surface: GovernanceDisclosureSurface::new(
                GovernanceDisclosureSurfaceKind::InjectedLocal,
    ...
    .expect("Core-owned visible route succeeds");
...
#[test]
fn core_owned_authoritative_visible_route_requires_delivery_capability() {
    let project = TestProject::new("core-owned-authoritative-visible-missing");
    project.write_core_owned_authoritative_visible_project();
    let skill_calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::clone(&skill_calls),
    ...
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id =
        WorkflowRunId::new("run-core-owned-authoritative-visible-missing").expect("run id");
    let request = project.core_owned_authoritative_request(run_id.clone());

    let error = route_core_owned_authoritative_explicit_local_check_profile_governance(
        &executor, &store, &profile, None, &request,
    )
    .expect_err("visible route without delivery capability fails closed");
...
fn core_owned_authoritative_approval_reassesses_the_same_immutable_shape_and_check() {
    let project = TestProject::new("core-owned-authoritative-approval");
    project.write_core_owned_authoritative_approval_project();
    let skill_calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::clone(&skill_calls),
    }));
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-core-owned-authoritative-approval").expect("run id");
    let execution = project.core_owned_authoritative_request(run_id.clone());
    let dependencies = LocalExecutionAuthoritativeVisibleGovernanceDependencies {
        disclosure: LocalExecutionGovernanceDisclosureInputs {
            delivery_id: GovernanceDisclosureDeliveryId::new(
                "delivery/core-owned-authoritative-approval",
            )
            .expect("delivery id"),
            surface: GovernanceDisclosureSurface::new(
                GovernanceDisclosureSurfaceKind::InjectedLocal,
    ...
    let paused = route_core_owned_authoritative_explicit_local_check_profile_governance(
        &executor,
        &store,
        &profile,
        Some(dependencies),
    ...
    .expect("immutable approval policy derives approval");
    assert!(matches!(
        paused,
        LocalExecutionWithAuthoritativeGovernanceRouteResult::ApprovalRequired(_)
    ));
    ...
    let presentation = approval_presentation_record(
        &approval,
        "presentation/core-owned-authoritative-approval",
        Timestamp::now_utc(),
    );
    backend
        .write_approval_presentation_record(&presentation)
    ...
    let mut report = report_inputs();
    report.local_check_result_references.clear();
    let request = LocalCoreOwnedAuthoritativeGovernanceApprovalReportDecisionRequest {
        approval: LocalApprovalPresentationDecisionRequest {
            approval: project.approval_request(
                run_id.clone(),
                approval.approval_id,
        ...
        report,
        local_check_reference: AuthoritativeDocsCheckReportReferenceInputs {
            result_id: LocalCheckResultId::new(
                "local-check-result/core-owned-authoritative-approval",
            )
            .expect("result id"),
            workflow_event_id: None,
            audit_event_id: None,
    ...
    let result =
        ...
        .expect("Core-owned approval reassessment succeeds");
...
#[test]
fn core_owned_authoritative_route_rejects_multi_step_before_check_or_run_state() {
    let project = TestProject::new("core-owned-authoritative-multi-step");
    project.write_authoritative_project_validation_project();
    project.declare_authoritative_project_validation();
    let skill_calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-core-owned-authoritative-multi-step").expect("run id");
    let request = project.core_owned_authoritative_request(run_id.clone());

    let error = route_core_owned_authoritative_explicit_local_check_profile_governance(
        &executor, &store, &profile, None, &request,
    )
    .expect_err("multi-step closed path fails");

    assert_eq!(
        error.code(),
        "executor.authoritative_local_check.workflow_shape_unsupported"
    );
...
#[test]
fn core_owned_authoritative_report_fails_closed_on_failed_same_call_check() {
    let project = TestProject::new("core-owned-authoritative-check-failed");
    project.write_core_owned_authoritative_project_validation_project();
    let skill_calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::clone(&skill_calls),
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-core-owned-authoritative-check-failed").expect("run id");
    let request = core_owned_authoritative_governance_report_request(
        project.core_owned_authoritative_request(run_id.clone()),
        "local-check-result/core-owned-authoritative-check-failed",
    );
    ...
    let error =
        ...
        .expect_err("failed same-call check blocks route");
...
#[test]
fn explicit_project_validation_profile_generates_report_from_same_call_check() {
    let project = TestProject::new("authoritative-project-validation-report");
    project.write_authoritative_project_validation_project();
    let skill_calls = Rc::new(Cell::new(0));
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::clone(&skill_calls),
    ...
    let store = LocalImmutableRunBundleStore::new(project.path().join("immutable-bundles"));
    let executor = LocalExecutor::new(&backend, &registry);
    let run_id = WorkflowRunId::new("run-authoritative-project-validation-report").expect("run id");
    let request = authoritative_governance_report_request(
        project.authoritative_docs_check_request(run_id.clone()),
        "local-check-result/authoritative-project-validation-report",
    );

    let result = execute_with_authoritative_explicit_local_check_profile_governance_report(
        &executor, &store, &profile, None, &request,
    )
    .expect("profile report consumer succeeds");
...
#[test]
fn provider_write_presentation_gate_not_required_preserves_provider_call() {
    let project = TestProject::new("executor-provider-write-presentation-not-required");
    project.write_approval_project();
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::new(Cell::new(0)),
    }));
...
#[test]
fn live_sandbox_approval_authority_derives_linked_approval_before_provider_call() {
    let project = TestProject::new("live-sandbox-approval-authority-success");
    let (backend, completed, approval) = approve_with_presentation_proof(
        &project,
        ApprovalDecisionKind::Granted,
        "presentation/live-sandbox-approval-authority-success",
    );
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::new(Cell::new(0)),
    }));
    ...
    let target_proof = live_sandbox_target_proof_for_executor();
    let request = live_sandbox_approval_authority_request(&target_proof, &attempted, &completed);
    let request_debug = format!("{request:?}");
    assert!(request_debug.contains("GitHubPrCommentLiveSandboxApprovalAuthorityCompositionRequest"));
    ...
    assert_eq!(
        result.live_sandbox().readiness().decision(),
        ProviderWriteSandboxReadinessDecision::AllowedForSandbox
    );
    let debug = format!("{result:?}");
    assert!(!debug.contains("approval/"));
...
#[test]
fn live_sandbox_approval_authority_missing_presentation_blocks_provider_call() {
    let project = TestProject::new("live-sandbox-approval-authority-missing-proof");
    project.write_approval_project();
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::new(Cell::new(0)),
    }));
    ...
    .expect_err("missing presentation proof blocks");
...
#[test]
fn live_sandbox_approval_authority_wrong_approval_identity_blocks_provider_call() {
    let project = TestProject::new("live-sandbox-approval-authority-wrong-approval");
    let (backend, completed, _approval) = approve_with_presentation_proof(
        &project,
        ApprovalDecisionKind::Granted,
        "presentation/live-sandbox-approval-authority-wrong-approval",
    );
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::new(Cell::new(0)),
    }));
    ...
    let attempted = github_pr_comment_attempted_record_for_provider_write_with_approval_ref(
        ...
        SideEffectReference::new(
            SideEffectReferenceKind::ApprovalDecision,
            "approval/not-presented-or-decided",
        )
        .expect("wrong approval reference"),
    );
    let target_proof = live_sandbox_target_proof_for_executor();
    let calls = AtomicU64::new(0);
    ...
    .expect_err("wrong approval identity blocks");
    ...
    assert!(!error.to_string().contains("not-presented-or-decided"));
}

#[test]
fn live_sandbox_approval_authority_missing_store_linkage_blocks_provider_call() {
    let project = TestProject::new("live-sandbox-approval-authority-missing-linkage");
    let (backend, completed, approval) = approve_with_presentation_proof(
        &project,
        ApprovalDecisionKind::Granted,
        "presentation/live-sandbox-approval-authority-missing-linkage",
    );
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::new(Cell::new(0)),
    }));
    ...
    let empty_store =
        LocalStateBackend::new(project.path().join(".empty-linkage-store")).expect("empty store");
    let target_proof = live_sandbox_target_proof_for_executor();
    let calls = AtomicU64::new(0);
    ...
    .expect_err("missing store linkage blocks");
...
fn live_sandbox_approval_authority_stale_presentation_blocks_provider_call() {
    ...
    let (backend, completed, approval) = approve_with_presentation_proof(
        &project,
        ApprovalDecisionKind::Granted,
        "presentation/live-sandbox-approval-authority-stale-proof",
    );
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::new(Cell::new(0)),
    }));
    ...
    let mut request =
        live_sandbox_approval_authority_request(&target_proof, &attempted, &completed);
    request.presentation_policy = request
        .presentation_policy
        .with_max_presentation_age(Duration::from_secs(1));
    let calls = AtomicU64::new(0);
    ...
    .expect_err("stale presentation blocks");
...
#[test]
fn live_sandbox_approval_authority_rejects_non_proof_policy_before_provider_call() {
    let project = TestProject::new("live-sandbox-approval-authority-policy-required");
    let (backend, completed, approval) = approve_with_presentation_proof(
        &project,
        ApprovalDecisionKind::Granted,
        "presentation/live-sandbox-approval-authority-policy-required",
    );
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::new(Cell::new(0)),
    }));
    ...
    let mut request =
        live_sandbox_approval_authority_request(&target_proof, &attempted, &completed);
    request.presentation_policy = ApprovalPresentationDefaultEnforcementPolicy::not_required();
    let calls = AtomicU64::new(0);
    ...
    .expect_err("non-proof policy blocks");
...
fn live_github_pr_comment_sandbox_composes_provider_outcome_and_durable_event_proof() {
    ...
    let (backend, completed, approval) = approve_with_presentation_proof(
        &project,
        ApprovalDecisionKind::Granted,
        "presentation/live-github-pr-comment-end-to-end-proof",
    );
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::new(Cell::new(0)),
    }));
    ...
    let mut request = live_sandbox_runtime_request_with_inputs(&target_proof, provider_inputs);
    ...
    let live_sandbox = compose_github_pr_comment_live_sandbox_runtime_with_approval_authority(
        &executor,
        &backend,
        &provider,
        GitHubPrCommentLiveSandboxApprovalAuthorityCompositionRequest {
            live_sandbox: request,
            run: &completed,
            presentation_policy:
                ApprovalPresentationDefaultEnforcementPolicy::required_for_sensitive_action(
                    LocalApprovalPresentationProof::ResolveByRunAndApproval,
    ...
    .expect("proof-bound live sandbox provider outcome composes");
    ...
    let (live_sandbox, _approval_linkage) = live_sandbox.into_parts();
...
#[test]
fn provider_write_presentation_gate_missing_approval_reference_blocks_provider_call() {
    let project = TestProject::new("executor-provider-write-presentation-missing-approval-ref");
    let (backend, completed, _approval) = approve_with_presentation_proof(
        &project,
        ApprovalDecisionKind::Granted,
        "presentation/provider-write-missing-approval-ref",
    );
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::new(Cell::new(0)),
    }));
    ...
    let attempted = github_pr_comment_attempted_record_for_provider_write_with_approval_ref(
        ...
        SideEffectReference::new(
            SideEffectReferenceKind::EvidenceReference,
            "evidence/provider-write-without-approval-reference",
        )
        .expect("non-approval reference"),
    );
    let request = LocalExecutionWithGitHubPrCommentProviderWritePresentationGateRequest {
        request: execution_with_github_pr_comment_provider_write_request(
            &project, run_id, &attempted,
    ...
    let error = result
        .provider_write_error()
        .expect("provider-write error explains missing approval reference");
    assert_eq!(
        error.code(),
    ...
    assert!(!error
        .to_string()
        .contains("provider-write-without-approval-reference"));
}

#[test]
fn provider_write_presentation_gate_stale_proof_blocks_provider_call() {
    let project = TestProject::new("executor-provider-write-presentation-stale-proof");
    let (backend, completed, approval) = approve_with_presentation_proof(
        &project,
        ApprovalDecisionKind::Granted,
        "presentation/provider-write-stale-proof",
    );
    let registry = registry(Box::new(EchoHandler {
        calls: Rc::new(Cell::new(0)),
    }));
    ...
    let error = result
        .provider_write_error()
        .expect("provider-write error explains stale proof");
    assert_eq!(
        error.code(),
        "approval_presentation_enforcement.proof_stale"
    );
    ...
    assert!(!error
        .to_string()
        .contains("presentation/provider-write-stale-proof"));
}
