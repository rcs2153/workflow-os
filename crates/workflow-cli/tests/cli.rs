...
fn init_repo_governance_creates_valid_local_project() {
    ...
    assert!(stdout(&output).contains("authoritative_execution: enabled"));
    let manifest =
...
#[test]
fn init_repo_governance_positive_flag_preserves_closed_authoritative_execution() {
...
#[test]
fn init_repo_governance_can_explicitly_select_legacy_scaffold() {
    let project = TestProject::new("repo-governance-legacy");

    let output = workflow_os(
        &project,
        &["init-repo-governance", "--no-authoritative-governance"],
    );

    assert!(output.status.success(), "{}", stderr(&output));
    assert!(stdout(&output).contains("authoritative_execution: disabled"));
    let manifest =
        fs::read_to_string(project.path().join("workflow-os.yml")).expect("manifest exists");
...
#[test]
fn init_repo_governance_rejects_contradictory_posture_before_writing() {
    let project = TestProject::new("repo-governance-contradictory-posture");
    ...
    assert!(!output.status.success());
    assert!(stderr(&output).contains("cli.init_repo_governance.authoritative_governance_conflict"));
    assert!(!project.path().join("workflow-os.yml").exists());
...
#[test]
fn init_repo_governance_legacy_generated_workflow_runs_to_approval_with_mock_skill() {
...
#[test]
fn init_repo_governance_authoritative_dry_run_discloses_compatibility_selection_without_writing() {
...
fn author_workflow_steward_review_approves_preflight_passing_draft_without_promotion() {
    ...
    assert!(out.contains("files_written: false"));
    assert!(out.contains("workflow_files_written: false"));
    assert!(out.contains("catalog_records_written: false"));
    assert!(out.contains("workflow_registered: false"));
    assert!(out.contains("workflow_promoted: false"));
    assert!(out.contains("approval_persisted: false"));
    assert!(out.contains("stewardship_persisted: false"));
    assert!(out.contains("commands_executed: false"));
    assert!(out.contains("providers_called: false"));
    assert!(out.contains("runtime_state_created: false"));
...
#[test]
fn author_workflow_steward_review_persists_one_catalog_stewardship_record_when_requested() {
    let project = TestProject::new("author-workflow-steward-review-persist");
    let init = workflow_os(&project, &["init-repo-governance"]);
    assert!(init.status.success(), "{}", stderr(&init));
    write_promotable_workflow_draft(&project);
    ...
    assert!(review.status.success(), "{}", stderr(&review));
    let out = stdout(&review);
    assert!(out.contains("mode: author_workflow_steward_review_persisted"));
    assert!(out.contains("files_written: true"));
    assert!(out.contains("workflow_files_written: false"));
    ...
    assert!(out.contains("workflow_promoted: false"));
    assert!(out.contains("approval_persisted: false"));
    assert!(out.contains("stewardship_persisted: true"));
    assert!(out.contains("stewardship_decision_id: stewardship/approved_for_promotion/"));
    assert!(out.contains("catalog_root: .workflow-os/catalog"));
    assert!(out.contains("commands_executed: false"));
    assert!(out.contains("providers_called: false"));
    assert!(out.contains("runtime_state_created: false"));
    ...
    let stewardship_dir = project.path().join(".workflow-os/catalog/stewardship");
    let records = fs::read_dir(&stewardship_dir)
        .expect("stewardship directory exists")
        .collect::<Result<Vec<_>, _>>()
        .expect("stewardship records can be read");
    assert_eq!(records.len(), 1);
    let record = fs::read_to_string(records[0].path()).expect("stewardship record is readable");
    assert!(record.contains(r#""decision_kind": "approved_for_promotion""#));
    assert!(record.contains(r#""workflow_id": "local/repo-implementation""#));
    assert!(record.contains(r#""draft_path": "workflows/drafts/repo-implementation.workflow.yml""#));
    assert!(record.contains(r#""candidate_content_hash""#));
    assert!(!record.contains("bounded-governed-implementation"));
}

#[test]
fn author_workflow_steward_review_persist_json_is_bounded() {
    let project = TestProject::new("author-workflow-steward-review-persist-json");
    let init = workflow_os(&project, &["init-repo-governance"]);
    assert!(init.status.success(), "{}", stderr(&init));
    write_promotable_workflow_draft(&project);
    ...
    assert!(review.status.success(), "{}", stderr(&review));
    let out = stdout(&review);
    assert!(out.contains(r#""mode":"author_workflow_steward_review_persisted""#));
    assert!(out.contains(r#""files_written":true"#));
    assert!(out.contains(r#""workflow_files_written":false"#));
    assert!(out.contains(r#""catalog_records_written":true"#));
    assert!(out.contains(r#""workflow_registered":false"#));
    assert!(out.contains(r#""workflow_promoted":false"#));
    assert!(out.contains(r#""approval_persisted":false"#));
    assert!(out.contains(r#""stewardship_persisted":true"#));
    assert!(out.contains(r#""stewardship_decision_id":"stewardship/approved_for_promotion/"#));
    assert!(out.contains(r#""catalog_root":".workflow-os/catalog""#));
    assert!(!out.contains("bounded-governed-implementation"));
    assert!(!out.contains("bounded steward review reason"));
}

#[test]
fn author_workflow_steward_review_catalog_root_requires_persistence() {
    let project = TestProject::new("author-workflow-steward-review-root-no-persist");
    let init = workflow_os(&project, &["init-repo-governance"]);
    assert!(init.status.success(), "{}", stderr(&init));
    write_promotable_workflow_draft(&project);
    ...
    assert!(!review.status.success());
    assert!(stderr(&review).contains("--catalog-root requires --persist-stewardship"));
    assert!(!project.path().join(".workflow-os/catalog").exists());
}

#[test]
fn author_workflow_steward_review_persist_rejects_unsafe_catalog_root_without_leakage() {
    let project = TestProject::new("author-workflow-steward-review-unsafe-root");
    let init = workflow_os(&project, &["init-repo-governance"]);
    assert!(init.status.success(), "{}", stderr(&init));
    write_promotable_workflow_draft(&project);
    ...
    assert!(!review.status.success());
    assert!(stderr(&review).contains("cli.workflow_catalog.catalog_root_rejected"));
    assert!(!stdout(&review).contains("secret-token-catalog"));
    assert!(!stderr(&review).contains("secret-token-catalog"));
    assert!(!project.path().join(".workflow-os/catalog").exists());
}

#[test]
fn author_workflow_steward_review_duplicate_persistence_fails_closed() {
    let project = TestProject::new("author-workflow-steward-review-duplicate-persist");
    let init = workflow_os(&project, &["init-repo-governance"]);
    assert!(init.status.success(), "{}", stderr(&init));
    write_promotable_workflow_draft(&project);
    ...
    let second = workflow_os(&project, &args);
    assert!(!second.status.success());
    assert!(stderr(&second).contains("cli.workflow_authoring.stewardship_persist_failed"));
    assert!(!stdout(&second).contains("bounded steward review reason"));
    assert!(!stderr(&second).contains("bounded steward review reason"));

    let records = fs::read_dir(project.path().join(".workflow-os/catalog/stewardship"))
        .expect("stewardship directory exists")
        .collect::<Result<Vec<_>, _>>()
        .expect("stewardship records can be read");
    assert_eq!(records.len(), 1);
...
#[test]
fn author_workflow_promote_dry_run_validates_without_writing() {
    let project = TestProject::new("author-workflow-promote-dry-run");
    let init = workflow_os(&project, &["init-repo-governance"]);
    assert!(init.status.success(), "{}", stderr(&init));
    write_promotable_workflow_draft(&project);
    ...
    assert!(out.contains("mode: author_workflow_active_promotion_dry_run"));
    assert!(out.contains("status: active_promotion_dry_run"));
    assert!(out.contains("active_workflow_path: workflows/repo-implementation.workflow.yml"));
    assert!(out.contains("candidate_workflow_id: local/repo-implementation"));
    assert!(out.contains("pre_write_active_context_validated: true"));
    assert!(out.contains("post_write_project_validation: not_run_dry_run"));
    assert!(out.contains("files_written: false"));
    assert!(out.contains("active_workflow_file_written: false"));
    assert!(out.contains("draft_preserved: true"));
    assert!(out.contains("workflow_promoted: false"));
    assert!(out.contains("approval_persisted: false"));
    assert!(out.contains("commands_executed: false"));
    assert!(out.contains("providers_called: false"));
...
#[test]
fn author_workflow_promote_writes_one_active_file_and_project_validates() {
    let project = TestProject::new("author-workflow-promote-success");
    let init = workflow_os(&project, &["init-repo-governance"]);
    assert!(init.status.success(), "{}", stderr(&init));
    write_promotable_workflow_draft(&project);
    ...
    assert!(out.contains("pre_write_active_context_validated: true"));
    assert!(out.contains("post_write_project_validation: passed"));
    assert!(out.contains("files_written: true"));
    assert!(out.contains("active_workflow_file_written: true"));
    assert!(out.contains("workflow_promoted: true"));
    assert!(out.contains("draft_preserved: true"));
    assert!(out.contains("approval_persisted: false"));
    assert!(out.contains("commands_executed: false"));
    assert!(out.contains("providers_called: false"));
...
#[test]
fn author_workflow_promote_json_is_bounded() {
    let project = TestProject::new("author-workflow-promote-json");
    let init = workflow_os(&project, &["init-repo-governance"]);
    assert!(init.status.success(), "{}", stderr(&init));
    write_promotable_workflow_draft(&project);
    ...
    assert!(promote.status.success(), "{}", stderr(&promote));
    let out = stdout(&promote);
    assert!(out.contains(r#""mode":"author_workflow_active_promotion_dry_run""#));
    assert!(out.contains(r#""status":"active_promotion_dry_run""#));
    assert!(out.contains(r#""active_workflow_path":"workflows/repo-implementation.workflow.yml""#));
    assert!(out.contains(r#""pre_write_active_context_validated":true"#));
    assert!(out.contains(r#""active_workflow_file_written":false"#));
    assert!(out.contains(r#""workflow_promoted":false"#));
    assert!(out.contains(r#""approval_persisted":false"#));
    assert!(!out.contains("bounded-governed-implementation"));
    assert!(!out.contains("bounded active promotion review"));
...
#[test]
fn author_workflow_promote_rejects_active_output_overwrite_before_writing() {
    let project = TestProject::new("author-workflow-promote-output-exists");
    let init = workflow_os(&project, &["init-repo-governance"]);
    assert!(init.status.success(), "{}", stderr(&init));
    write_promotable_workflow_draft(&project);
    project.write(
...
version: v0
display_name: Existing Active
description: Existing active workflow occupying the promotion output path.
owner:
  owning_team: workflow-stewards
  maintainer: user/workflow-steward
  escalation_contact: user/workflow-escalation
...
",
    ...
    assert!(!promote.status.success());
    assert!(
        stderr(&promote).contains("cli.workflow_authoring.active_promotion_output_exists"),
        "{}",
        stderr(&promote)
    );
    assert!(!stderr(&promote).contains("bounded-governed-implementation"));
    assert!(!project.state_root().exists());
}

#[test]
fn author_workflow_promote_blocks_failed_preflight_before_writing() {
    let project = TestProject::new("author-workflow-promote-blocked");
    let init = workflow_os(&project, &["init-repo-governance"]);
    assert!(init.status.success(), "{}", stderr(&init));
    let write = workflow_os(
        &project,
    ...
    assert!(!promote.status.success());
    let out = stdout(&promote);
    assert!(out.contains("status: active_promotion_blocked"));
    assert!(out.contains("pre_write_active_context_validated: false"));
    assert!(out.contains("workflow_id_still_draft_namespace"));
    assert!(out.contains("files_written: false"));
    assert!(out.contains("workflow_promoted: false"));
    assert!(stderr(&promote).contains("cli.workflow_authoring.active_promotion_blocked"));
    assert!(!project
        .path()
        .join("workflows/repo-implementation.workflow.yml")
        .exists());
...
#[test]
fn author_workflow_promote_rejects_secret_like_reason_without_leakage() {
    let project = TestProject::new("author-workflow-promote-secret-reason");
    let init = workflow_os(&project, &["init-repo-governance"]);
    assert!(init.status.success(), "{}", stderr(&init));
    write_promotable_workflow_draft(&project);
    ...
    assert!(!promote.status.success());
    assert!(stderr(&promote).contains("workflow_authoring.steward_review.secret_like_value"));
    assert!(!stdout(&promote).contains("contains-secret-value"));
    assert!(!stderr(&promote).contains("contains-secret-value"));
    assert!(!project
        .path()
        .join("workflows/repo-implementation.workflow.yml")
        .exists());
