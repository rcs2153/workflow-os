...
Status: Explicit `DocsCheck` registry helper implemented. `DocsCheckLocalHandler` remains production-shaped, explicit, non-default, and non-CLI. Local check result citation planning is documented in [Local Check Result Citation Plan](local-check-result-citation-plan.md). This plan does not implement default registration, CLI exposure, workflow schema fields, automatic check execution, `AllowlistedHandlerOnly`, report artifacts, local check result citation, evidence attachment, command-output evidence, side-effect boundary implementation, source writes, or release posture changes.
