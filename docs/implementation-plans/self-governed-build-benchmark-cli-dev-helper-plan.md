...
Status: Implemented as a repo-local development helper in `scripts/self-governed-benchmark.mjs` with npm alias `dogfood:benchmark`. The implementation is reported in [Self-Governed Build Benchmark CLI/Dev-Helper Report](../concepts/SELF_GOVERNED_BUILD_BENCHMARK_CLI_DEV_HELPER_REPORT.md) and accepted in [Self-Governed Build Benchmark CLI/Dev-Helper Review](../concepts/SELF_GOVERNED_BUILD_BENCHMARK_CLI_DEV_HELPER_REVIEW.md). This plan follows the accepted [Self-Governed Build Benchmark Behavior Test Review](../concepts/SELF_GOVERNED_BUILD_BENCHMARK_BEHAVIOR_TEST_REVIEW.md). It does not add stable public CLI behavior, change runtime semantics, register default local check handlers, write report artifacts, add schemas, execute arbitrary commands, add writes, implement reasoning lineage, or change release posture.
