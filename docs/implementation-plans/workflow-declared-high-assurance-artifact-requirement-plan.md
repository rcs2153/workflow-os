...
Status: First internal model and policy-mapping slice implemented in [Workflow-Declared High-Assurance Artifact Requirement Model Report](../concepts/WORKFLOW_DECLARED_HIGH_ASSURANCE_ARTIFACT_REQUIREMENT_MODEL_REPORT.md). Workflow schema fields, workflow-declared parsing, runtime config, automatic report generation, automatic artifact writing, CLI behavior, examples, RBAC/IdP, quorum approval, revocation enforcement, side-effect execution, write-capable adapters, hosted behavior, reasoning lineage, and release posture changes are not implemented.
