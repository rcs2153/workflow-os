...
Status: Validation-only helper implemented in [Report Artifact SideEffect Referential Integrity Report](../concepts/REPORT_ARTIFACT_SIDE_EFFECT_REFERENTIAL_INTEGRITY_REPORT.md) and accepted in [Report Artifact SideEffect Referential Integrity Review](../concepts/REPORT_ARTIFACT_SIDE_EFFECT_REFERENTIAL_INTEGRITY_REVIEW.md). A later explicit executor-adjacent artifact write path now composes report generation, SideEffect referential integrity, and approval-linkage gates in [Executor Report Artifact SideEffect Gates Report](../concepts/EXECUTOR_REPORT_ARTIFACT_SIDE_EFFECT_GATES_REPORT.md). This plan follows the accepted [Executor SideEffect Discovery Opt-In Review](../concepts/EXECUTOR_SIDE_EFFECT_DISCOVERY_OPT_IN_REVIEW.md). It does not implement automatic artifact writes from existing executor paths, automatic SideEffect discovery, runtime side-effect execution, writes, schemas, CLI behavior, examples, hosted behavior, reasoning lineage, or release posture changes.
