...
Status: Implemented as model-only core types in `workflow-core` and accepted in [SideEffect Core Model Review](../concepts/SIDE_EFFECT_CORE_MODEL_REVIEW.md). ADR 0011 is accepted as architecture direction. WorkReport SideEffect citation vocabulary, terminal report helper propagation, and executor report input propagation are also accepted. SideEffect workflow event and audit projection planning is documented in [SideEffect Workflow Event And Audit Projection Plan](side-effect-workflow-event-audit-projection-plan.md). This plan does not implement writes, write-capable adapters, generic runtime adapter execution, workflow events, audit projections, persistence changes, schemas, CLI behavior, examples, hosted runtime behavior, or release posture changes.
