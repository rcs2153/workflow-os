...
It does not answer every governance question. Work report models and explicit local report helper/artifact APIs are implemented. Side-effect boundary architecture is proposed in [ADR 0011: Side-Effect Boundary Core Model](../adr/0011-side-effect-boundary.md), but the side-effect core model, writes, persistence, schemas, CLI behavior, and runtime side-effect execution are not implemented. Additional adapter/validation/approval attachment points, evidence persistence, CLI inspection, and reasoning lineage remain future scoped work.
