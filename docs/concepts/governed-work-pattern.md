...
SideEffect approval linkage is implemented as explicit validation helpers only. The in-memory helper validates already-loaded `SideEffectRecord` values against approval events from the same immutable `WorkflowRun`, and the store-backed helper loads caller-selected records through a supplied `SideEffectRecordStore` before delegating to that same validation boundary.

These helpers prove approval authority references; they do not execute side effects, mutate providers, write report artifacts, run automatically in executor/report paths, attach approval evidence, add schemas, expose CLI behavior, or authorize write-capable adapters.
