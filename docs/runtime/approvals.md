...
The implemented helper is explicit, local, and in-memory. It can validate selected controls and supplied stable references without mutating workflow state. It is not wired into the default `decide_approval(...)` path. Executor-integrated enforcement is planned as a future opt-in method that validates before appending approval decision events while leaving the default path unchanged. This is not a safety-critical certification claim. Later scoped runtime phases must be implemented and reviewed before write-capable adapters or high-risk external actions depend on it.
