...
Open P0 hardening gap: [Approval Gate Presentation Enforcement Gap](../concepts/APPROVAL_GATE_PRESENTATION_ENFORCEMENT_GAP.md) tracks the remaining proof boundary. The helper emits the approval content, and repo instructions require agents to present it, but the dogfood runner does not yet durably record the exact approval text/card shown before approval. Dogfood runner proof persistence planning is documented in [Dogfood Runner Approval-Presentation Persistence Plan](../implementation-plans/dogfood-runner-approval-presentation-persistence-plan.md). Future enforcement should persist a typed, redaction-safe approval-presentation record during `phase-start` and allow material approvals to fail closed when the presentation record is missing or stale.
