...
P0 hardening status: [Approval Gate Presentation Enforcement Gap](../concepts/APPROVAL_GATE_PRESENTATION_ENFORCEMENT_GAP.md) tracks the proof boundary. The helper emits the approval content, repo instructions require agents to present it, and the dogfood runner now persists a typed, redaction-safe approval-presentation record during material `phase-start` runs before approval. Dogfood runner proof persistence is documented in [Dogfood Runner Approval-Presentation Persistence Plan](../implementation-plans/dogfood-runner-approval-presentation-persistence-plan.md). Dogfood approval enforcement is implemented in [Dogfood Runner Approval-Presentation Enforcement Plan](../implementation-plans/dogfood-runner-approval-presentation-enforcement-plan.md): material phase-start output now prints a proof-enforced dogfood approval command that passes the persisted `presentation_id` and a bounded max-age freshness policy into the existing opt-in enforcement path. Freshness enforcement is documented in [Dogfood Approval-Presentation Freshness Enforcement Report](../concepts/DOGFOOD_APPROVAL_PRESENTATION_FRESHNESS_ENFORCEMENT_REPORT.md). Dogfood phase-close proof disclosure is implemented in [Dogfood Phase-Close Proof-Enforcement Disclosure Report](../concepts/DOGFOOD_PHASE_CLOSE_PROOF_ENFORCEMENT_DISCLOSURE_REPORT.md), and bounded proof-marker inspect/projection is implemented in [Approval Event Proof Marker Inspect Projection Report](../concepts/APPROVAL_EVENT_PROOF_MARKER_INSPECT_PROJECTION_REPORT.md): `phase-close` now reports matching approval-presentation proof-record status, bounded proof IDs, and `proof_enforced` when inspect output exposes an approval decision proof marker. Public approval behavior remains unchanged.
