...
P0 hardening status: [Approval Gate Presentation Enforcement Gap](../concepts/APPROVAL_GATE_PRESENTATION_ENFORCEMENT_GAP.md) tracks the proof boundary. The helper emits the approval content, repo instructions require agents to present it, and the dogfood runner now persists a typed, redaction-safe approval-presentation record during material `phase-start` runs before approval. Dogfood runner proof persistence is documented in [Dogfood Runner Approval-Presentation Persistence Plan](../implementation-plans/dogfood-runner-approval-presentation-persistence-plan.md). Dogfood approval enforcement is planned in [Dogfood Runner Approval-Presentation Enforcement Plan](../implementation-plans/dogfood-runner-approval-presentation-enforcement-plan.md): the next implementation should make dogfood approvals call the opt-in presentation-enforcement path by default and fail closed when proof is missing or stale. Until then, persisted proof is available for audit, but ordinary dogfood approval commands remain explicit and non-enforcing.
