...
Diagnostics are printed for validation failures where available. Experimental preview JSON output is available for selected commands with `--json`, but it remains not a versioned stable machine-output contract through `0.2.0-preview.1`.
