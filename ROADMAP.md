...
The next write-candidate work should review the explicit store-backed SideEffect attempted/completed/failed lifecycle transition helpers before any executor append or provider-write planning proceeds. The helpers load prior records from `SideEffectRecordStore`, reuse the pure transition helpers, update the stored record through an explicit replace boundary, and return reference-only event payloads for future executor integration. This remains pre-write work: provider writes, live GitHub comment creation, runtime side-effect execution, executor event append behavior, CLI mutation commands, schemas, examples, hosted behavior, reasoning lineage, recursive agents, agent swarms, Level 3/4 autonomy, and release posture changes remain out of scope.
