...
Real-repository onboarding evaluation found that this loop is valuable but still too generic for normal repositories. The next P0 UX lane is planned in [Real-Repo Onboarding UX Plan](docs/implementation-plans/real-repo-onboarding-ux-plan.md): preserve existing `AGENTS.md` content by default instead of pushing users toward destructive `--force`, inspect only safe repository metadata such as manifests and conventional directory presence, make recommendations concrete without executing commands, and label the generated mock workflow as an optional approval/audit demonstration rather than repository analysis.
