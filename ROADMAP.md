...
Side-effect boundary modeling must be accepted before policy-gated writes, generic runtime adapter execution, or domain packs. [ADR 0011: Side-Effect Boundary Core Model](docs/adr/0011-side-effect-boundary.md) is proposed as the domain-neutral architecture boundary for side-effect intent, authority, lifecycle state, idempotency, audit, evidence, and report citation. ADR 0011 does not implement side-effect model code, writes, provider mutations, schemas, CLI behavior, persistence changes, examples, hosted behavior, or release posture changes.
