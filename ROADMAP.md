...
The next write-candidate work should plan SideEffect attempted/completed/failed lifecycle transition mechanics before any live mutation implementation. The provider write readiness plan is accepted and defines the remaining gates for any future live sandbox helper, including explicit API posture, sandbox mode, policy, proposed SideEffect persistence, proposed event proof, approval/high-assurance posture, idempotency, credential handling, lifecycle transitions, audit/report disclosure, and failure semantics. It keeps provider writes, live GitHub comment creation, runtime side-effect execution, CLI mutation commands, schemas, examples, hosted behavior, reasoning lineage, recursive agents, agent swarms, Level 3/4 autonomy, and release posture changes out of scope.
